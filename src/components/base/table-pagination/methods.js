export default {
  ...
  resizeTableHeight () {
    ...
    // 不在窗口改变时获取，从内存中取出
    let tableWrapper = this.tableWrapper
    let top = $(tableWrapper).offset().top
    // debugger
    // 窗口高度 - 表格离窗口顶部的距离 - 偏差
    this.tableHeight = (window.document.body.offsetHeight - top - 141 - this.offset)
  },
  handleCurrentChange () {
    // 广播页码
    let param = {
    ...
    this.$emit('current-change', param)
  },
