export default {
  ...
  async deleteRow (scope) {
    let res = await this.request('deleteParking', {parkingId: scope.row.parkingId})
    if (res) { // 成功
  ...
  saveEdit (scope) {
    this.$confirm('确定保存该条修改？', {title: '提示'}).then(async () => {
      let res = await this.request('editParking', this.editRowData)
      if (res) { // 成功
  ...
  handleCurrentChange (payload) {
    ...
    this.methods('getParkingInfoList')
  },
  async getParkingInfoList (payload) {
    let res = await this.request('pageListParkingInfo', this.requestData)
  ...
  async clickConfirm () {
    this.$refs.elForm.validate(async (valid) => {
      if (valid) {
        let res = await this.request('addParking', this.appendData)
        if (res) { // 成功
