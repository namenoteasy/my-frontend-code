<template>
  <t-container title="泊位信息">
    ...
    <t-search>
      <el-form :inline="true" :model="requestData" label-width="70px" slot="searchLeft">
        ...
        <el-form-item label="泊位号">
          <el-select v-model="requestData.berthCode">

          </el-select>
        </el-form-item>
      </el-form>
      <div  slot="searchRight">
        <el-button type="primary" @click="clickGetBerthInfoList">搜索</el-button>
      </div>
    </t-search>
    <!--/搜索-->
    <div class="mt-17">
      ...
      <t-table-pagination
        ...
        <template slot="berthName" slot-scope="scope">
          <el-input v-if="scope.row.editable" v-model="editRowData.berthName" style="line-height: 34px"></el-input>
          <span v-else>{{scope.row.berthName}}</span>
        </template>
        <template slot="berthCode" slot-scope="scope">
          <el-input v-if="scope.row.editable" v-model="editRowData.berthCode" style="line-height: 34px"></el-input>
          <span v-else>{{scope.row.berthCode}}</span>
        </template>
    ...
    <t-dialog title="设备接入" width="465px" :show.sync="dialogVisible">
      <el-form ref="elForm" :inline="true" :model="requestData" :rules="rules" label-width="114px">
        ...
        <el-form-item label="名称" prop="berthName">
          <div class="long-input">
            <el-select v-model="appendData.berthName">
        ...
        <el-form-item label="泊位编号" prop="berthCode">
          <div class="long-input">
            <el-select v-model="appendData.berthCode">
